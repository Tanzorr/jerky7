...
class WayForPay
{
    ...
    {
        $this->_merchant_account = env('MERCHANT_LOGIN');
        $this->_merchant_password = env('SECRET_KEY');
    ...
    {
        $successUrl = $params['success_url'] ?? getUrl('/success-pay');
        $failUrl = $params['fail_url'] ?? '';

        $form = $this->buildForm([
            'merchantDomainName' => trim(getUrl('/'), '/'),
            'orderReference' => $params['nOrder'],
            ...
            'serviceUrl' => route('check-wayforpay'),
        ]);

        return view('pages.wayforpay', compact('form'));
    }
